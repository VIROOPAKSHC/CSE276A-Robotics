...
class EkfSlamNode(Node):
    ...
    def compute_and_publish_robot_pose_from_tag(self, tag_id, tag_observation):
        ...
        # Observation from camera to tag
        obs_pos = np.array([
        ...
        # Get the direction vector of camera's Z-axis (forward) in map frame
        camera_forward_in_map = camera_map_rot.apply([0, 0, 1])
        
        # Robot's yaw is the angle this forward direction makes in the XY plane
        yaw = np.arctan2(camera_forward_in_map[1], camera_forward_in_map[0])
        
        # Update pose estimate
        self.x_est = np.array([camera_map_pos[0], camera_map_pos[1], yaw])
        
        pose_change = self.x_est - old_pose
        self.get_logger().info(
            f'Tag {tag_id} update: pos=({self.x_est[0]:.3f}, {self.x_est[1]:.3f}), '
            f'yaw={math.degrees(self.x_est[2]):.1f}° | '
            f'Change: Δx={pose_change[0]:.3f}, Δy={pose_change[1]:.3f}, Δθ={math.degrees(pose_change[2]):.1f}°'
        )
    ...
    def control_loop(self):
        """Main control loop: Update localization, then navigate"""
        # First, update pose estimate using tag localization
        if self.cnt == 0:
            # self.localization_update()
            self.cnt = MAX_CNT
        
        self.cnt -= 1
        # If no tag detected, use dead reckoning
        # if not self.pose_updated_this_cycle:
        self.update_dead_reckoning()
        
        # Navigate based on updated pose
        self.navigate()
    ...
    def send_motor_commands(self, v_req, omega_req):
        """Send motor commands and update tracking variables"""
        l_wheel, r_wheel = self._get_wheel_speeds(v_req, omega_req)
        l_wheel_cmd = l_wheel * self.K_L
        r_wheel_cmd = r_wheel * self.K_R
        
        self.get_logger().info(
            f"Commands: v={v_req:.3f} m/s, ω={omega_req:.3f} rad/s | "
        ...
        # Update tracking
        self.last_control = np.array([v_req, omega_req])
        self.last_command_time = time.time()
